    ...
    def load_config():
        ...
        try:
            ...
            if not db_config:
                ...
                db_auth = db_config.get("auth", {})
                cfg.auth__host = db_auth.get("host") or cfg.auth__host
                cfg.auth__port = db_auth.get("port")  or cfg.auth__port

                db_orch = db_config.get("orch", {})
                cfg.orch__host = db_orch.get("host") or cfg.orch__host
                cfg.orch__port = db_orch.get("port")  or cfg.orch__port
...
#             mongo_client.close()
            ...
            LOGGER.error(
                "Deleting database [{}] failed: [{}]".format(
                    cfg.database__database_name, err
                )
            )
